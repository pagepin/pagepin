...
- **可插拔认证** —— 内置邮箱/密码（可选开放注册）、任意 OIDC provider，或 `none`（本地开发）。
...
| `PAGEPIN_ALLOW_SIGNUP` | `true` | 是否允许自助注册（password 模式）。 |
...
一个 Node 进程（Hono）+ SQLite + 可插拔对象存储，对外服务三样东西：JSON API、React 控制台，以及托管站点（向其 HTML 注入评论 overlay）。
