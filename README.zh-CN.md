...
- **分享链接与访客评审** —— 生成聊天友好的短链接 `…/s/<code>`（`POST /api/sites/{slug}/share-link`）；任何人点开即可查看私有页面并**以访客身份打点评论，无需注册账号**。链接默认永不过期（可选 `hours` 限时），可列出、单条撤销或一键全部撤销（连同已进入的访客会话）；每站点可单独开关访客评论。
