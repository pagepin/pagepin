...
test('评论模式两步式:空草稿点别处只收掉,再点才开新;有稿点别处抖动保留;⌥+点击免清直接弹', async ({ page }) => {
  await setup(page, { html: IMG_HTML });
  await goto(page);
  await ready(page);
  await page.keyboard.press('c');
  ...
  await expect(draft(page)).toBeVisible();
  await page.mouse.click(560, 620); // 空白:空草稿 → 只关闭
  await expect(draft(page)).toHaveCount(0);
  await page.mouse.click(560, 620); // 再点才开新的
  await expect(draft(page)).toBeVisible();
  await draftTa(page).fill('还没写完'); // 有草稿时点别处:抖动拦截、保留
  await page.mouse.click(150, 150);
  await expect(draft(page)).toBeVisible();
  await page.keyboard.press('Escape'); // Esc 放弃草稿
  await expect(draft(page)).toHaveCount(0);
  await page.keyboard.down('Alt'); // ⌥+点击免清直接弹
