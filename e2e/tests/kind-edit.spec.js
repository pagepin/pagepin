// @ts-check
// 评论 kind 在弹层里可改(补 3b 遗留):点 chip → PATCH {kind} → pin/accent 重新上色;
// 再点同一项 → 取消(kind=null),回中性色。后端 PATCH /api/comments/threads/:tid 已支持可选 kind。
const { test, expect } = require('@playwright/test');
const { setup, goto, mkThread, popover } = require('./_helpers');
...
test('弹层点 kind chip:PATCH {kind} + pin 重新上色;再点取消回中性', async ({ page }) => {
  const patches = [];
  ...
  // 选 Bug
  await popover(page).locator('[data-pp-kind="bug"]').click();
  await expect.poll(() => patches.at(-1) && patches.at(-1).kind).toBe('bug');
  await expect(popover(page).locator('[data-pp-kind="bug"]')).toHaveClass(/pp-anno-on/);
  await expect.poll(() => pinColor(page)).toBe(BUG_RGB); // pin 重新上色为 Bug

  // 再点同项 → 取消(kind=null)
  await popover(page).locator('[data-pp-kind="bug"]').click();
  await expect.poll(() => patches.at(-1) && patches.at(-1).kind).toBe(null);
  await expect(popover(page).locator('[data-pp-kind="bug"]')).not.toHaveClass(/pp-anno-on/);
  await expect.poll(() => pinColor(page)).toBe(NO_KIND_RGB);
});

test('改 kind 不影响 resolved:弹层仍开,可继续 Resolve & next', async ({ page }) => {
  ...
  await page.locator('.pp-anno-pin').click();
  await expect(popover(page)).toBeVisible();
  await popover(page).locator('[data-pp-kind="question"]').click();
  await expect(popover(page).locator('[data-pp-kind="question"]')).toHaveClass(/pp-anno-on/);
  // 弹层未被关闭,reply 框仍在
  await expect(popover(page).locator('textarea')).toBeVisible();
});
