...
test('TOC:h2 少于 2 个不出;≥2 出左栏,滚动后 scroll-spy 高亮跟进(宽视口)', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 800 });
  await setup(page);
  const toc = page.locator('.pp-md-toc');
  await expect(toc).toBeVisible();
  await expect(toc.locator('a')).toHaveCount(4);
  await expect(toc.locator('a.pp-on')).toHaveText(/功能清单/); // 初始:首个标题
  ...
  await expect(toc.locator('a.pp-on')).toHaveText(/第四节/);
});
