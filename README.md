...
- **Share links & guest review** — mint a short `…/s/<code>` link (`POST /api/sites/{slug}/share-link`); anyone opening it can view the private page and **pin comments as a guest, no account needed**. Links never expire by default (optional `hours`); list them, revoke one, or revoke all (kicking guest sessions) — and toggle guest commenting per site.
