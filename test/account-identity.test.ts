...
async function setup(env: Record<string, string> = {}, mailer?: Mailer) {
  ...
  const db = await makeTestDb(); // 自动应用 drizzle 迁移(含 0005 identities)
...
test('federated: 同 (provider,sub) 二次登录 → 同一账号,不重复建号', async () => {
  ...
  const a = await upsertFederatedUser(deps, {
    provider: 'google',
    sub: 'google:1',
    email: 'a@x.com',
    emailVerified: true,
  ...
  const b = await upsertFederatedUser(deps, {
    provider: 'google',
    sub: 'google:1',
    email: 'a@x.com',
    emailVerified: true,
  ...
  const ids = await db.select().from(identities).where(eq(identities.userId, a.id));
...
test('federated: 同一 verified email 跨 provider → 自动合并进同一账号', async () => {
  ...
  const g = await upsertFederatedUser(deps, {
    provider: 'google',
    sub: 'google:1',
    email: 'same@x.com',
    emailVerified: true,
  });
  const h = await upsertFederatedUser(deps, {
    provider: 'github',
    sub: 'github:9',
    email: 'same@x.com',
    emailVerified: true,
  ...
  assert.equal((await db.select().from(users)).length, 1);
  const ids = await db.select().from(identities).where(eq(identities.userId, g.id));
  assert.deepEqual(ids.map((i) => i.provider).sort(), ['github', 'google']);
});

test('federated: 未验证 email 不作账号键', async () => {
  const { deps, db } = await setup();
  const u = await upsertFederatedUser(deps, {
    provider: 'oidc',
    sub: 'sub-1',
    email: 'x@y.com',
    emailVerified: false,
  });
  assert.equal((await db.select().from(users).where(eq(users.id, u.id)))[0]?.canonicalEmail, null);
  assert.equal(
    (await db.select().from(identities).where(eq(identities.userId, u.id)))[0]?.email,
    null,
  );
});
...
test('password: 注册建 user + password identity,大小写归一,变体重复 → 409', async () => {
  ...
  const r1 = await postJson(app, '/auth/signup', {
  ...
  const r2 = await postJson(app, '/auth/signup', {
    email: 'BOB@example.COM',
    password: 'password123',
  });
...
test('password: 登录按 canonicalEmail(大小写无关)', async () => {
  ...
  const r = await postJson(app, '/auth/password', {
...
test('attach: 把第二个 provider 连接到已登录账号 → 同账号多身份', async () => {
  ...
  assert.equal(
    await attachIdentity(deps, u.id, {
      provider: 'github',
      sub: 'github:2',
      email: 'a@x.com',
      emailVerified: true,
  ...
  const ids = await db.select().from(identities).where(eq(identities.userId, u.id));
...
test('attach: 幂等(已属于本账号)与冲突(属于别的账号)', async () => {
  ...
  assert.equal(
    await attachIdentity(deps, a.id, { provider: 'github', sub: 'github:2' }),
    'conflict',
  ); // 已绑别人
});
...
test('disconnect: password 不可断开;社交可断开(非最后一个),断开 bump epoch', async () => {
  ...
  const list = (await (
    await me.fetch(new Request('http://localhost/api/me/identities'))
  ).json()) as {
...
test('auto-link: 两边邮箱都已验证 → 社交登录直接进同一账号', async () => {
  ...
  const u = (
    await db.select().from(users).where(eq(users.canonicalEmail, 'henry@example.com'))
  )[0]!;
  await db.update(users).set({ emailVerified: true }).where(eq(users.id, u.id)); // 模拟已验证邮箱
  const r = await upsertFederatedUser(deps, {
    provider: 'github',
    sub: 'github:henry',
  ...
  assert.equal((await db.select().from(users)).length, 1); // 没新建
  const ids = await db.select().from(identities).where(eq(identities.userId, u.id));
...
test('auto-link 不触发:已有账号邮箱未验证 → 仍建独立账号(挡抢注)', async () => {
  ...
  const before = (await db.select().from(users))[0]!;
  const r = await upsertFederatedUser(deps, {
    provider: 'github',
    sub: 'github:iris',
  ...
  assert.equal((await db.select().from(users)).length, 2); // 独立账号
...
test('verify-email: 注册发验证信,点链接置 emailVerified(user + password identity)', async () => {
  ...
  let u = (await db.select().from(users).where(eq(users.canonicalEmail, 'frank@example.com')))[0]!;
  ...
  assert.equal(
    (await app.fetch(new Request(`http://localhost/auth/verify-email?token=${tok}`))).status,
    200,
  );

  u = (await db.select().from(users).where(eq(users.id, u.id)))[0]!;
...
test('verify-email: 篡改/过期 token → 400,不改状态', async () => {
  ...
  const r = await app.fetch(
    new Request('http://localhost/auth/verify-email?token=not-a-real-token'),
  );
  assert.equal(r.status, 400);
  const u = (
    await db.select().from(users).where(eq(users.canonicalEmail, 'grace@example.com'))
  )[0]!;
...
test('gate: 非 admin 未验证 password + 配邮件 → canPublish false;验证后 true', async () => {
  ...
  const u = (await db.select().from(users).where(eq(users.canonicalEmail, 'user@example.com')))[0]!;
...
test('gate: 无邮件实例 no-op;有 IdP 验证社交身份 → 放行', async () => {
  ...
  const ub = (
    await noMail.db.select().from(users).where(eq(users.canonicalEmail, 'b@example.com'))
  )[0]!;
  ...
  const us = (
    await wm.db.select().from(users).where(eq(users.canonicalEmail, 'soc@example.com'))
  )[0]!;
  assert.equal(await canPublish(wm.deps, us), false); // 未验证
  await attachIdentity(wm.deps, us.id, {
    provider: 'google',
    sub: 'google:soc',
...
test('reconcile: 验证时把【空】password 账号并入【有内容】的社交账号(主 bug 修复)', async () => {
  ...
  const A = (await db.select().from(users).where(eq(users.canonicalEmail, 'zoe@example.com')))[0]!;
  ...
  // B 有内容:handle + 站点
  await db.update(users).set({ handle: 'zoe' }).where(eq(users.id, B.id));
  ...
  await db.update(users).set({ emailVerified: true }).where(eq(users.id, A.id));
  await reconcileByVerifiedEmail(deps, 'zoe@example.com');

  const aRow = (await db.select().from(users).where(eq(users.id, A.id)))[0]!;
  const bRow = (await db.select().from(users).where(eq(users.id, B.id)))[0]!;
  ...
  const bIds = await db.select().from(identities).where(eq(identities.userId, B.id));
...
test('reconcile: 两边都有内容(两个 handle)→ 记 conflict,绝不自动合并', async () => {
  ...
  await db.update(users).set({ handle: 'aaa' }).where(eq(users.id, a.id));
  await db.insert(sites).values({
    id: uuid(),
    ownerId: a.id,
    ownerHandle: 'aaa',
  ...
  const bId = uuid();
  await db.insert(users).values({ id: bId, email: 't@x.com', handle: 'bbb', createdAt: nowIso() });
  ...
  assert.equal((await db.select().from(users).where(eq(users.id, a.id)))[0]!.disabled, false); // 两边都活
  assert.equal((await db.select().from(users).where(eq(users.id, bId)))[0]!.disabled, false);
  const conflicts = await db
    .select()
    .from(accountMerges)
    .where(eq(accountMerges.status, 'conflict'));
