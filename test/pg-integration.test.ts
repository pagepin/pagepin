...
test(
  'postgres end-to-end: migrations + dialect-switched tables + core semantics',
  { skip: !PG_URL },
  async () => {
    ...
    await db
      .insert(users)
      .values({ id: 'u1', handle: 'alice', createdAt: new Date().toISOString() });
    ...
    assert.equal(
      conflict.length,
      0,
      'guarded CAS update returns [] when the guard fails (conflict)',
    );

    // 7) boolean 列映射成 JS true
    const b = (
      await db.select({ ce: sites.commentsEnabled }).from(sites).where(eq(sites.id, 's3'))
    )[0];
    ...
    await upsert(
      db.insert(instanceSettings).values({ key: 'k', value: 'a' }),
      instanceSettings.key,
      { value: 'a' },
    );
    await upsert(
      db.insert(instanceSettings).values({ key: 'k', value: 'b' }),
      instanceSettings.key,
      { value: 'b' },
    );
    const setting = (
      await db.select().from(instanceSettings).where(eq(instanceSettings.key, 'k'))
    )[0];
