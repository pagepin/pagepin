...
export async function sendVerificationEmail(
  ...
  await mailer.send({
    ...
    text: t(locale, 'email.verify.text', { link }),
    html: verifyHtml(link, locale),
  });
  return true;
}
...
function verifyHtml(link: string, locale: Locale): string {
  return `<!doctype html><html lang="${locale}"><body style="margin:0;background:#ECEEEF;font-family:-apple-system,Segoe UI,Roboto,sans-serif">
...
<h1 style="margin:0 0 8px;font-size:19px;color:#11161b">${t(locale, 'email.verify.html.heading')}</h1>
<p style="margin:0 0 22px;font-size:14px;line-height:1.6;color:#6b7480">${t(locale, 'email.verify.html.body')}</p>
<a href="${link}" style="display:inline-block;background:#0f7c72;color:#fff;text-decoration:none;font-size:14px;font-weight:600;padding:11px 22px;border-radius:9px">${t(locale, 'email.verify.html.button')}</a>
<p style="margin:22px 0 0;font-size:12px;line-height:1.6;color:#9aa1a9;word-break:break-all">${t(locale, 'email.verify.html.orPaste')}<br>${link}</p>
<p style="margin:18px 0 0;font-size:12px;color:#9aa1a9">${t(locale, 'email.verify.html.ignore')}</p>
