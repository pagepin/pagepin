...
export const common: Record<Locale, Record<string, string>> = {
  en: {
    ...
    'comment.quote.notString': 'quote must be a string',
    'comment.quote.length': 'quote length must be {min}–{max} chars',
  ...
  zh: {
    ...
    'comment.quote.notString': 'quote 必须是字符串',
    'comment.quote.length': 'quote 长度须在 {min}–{max} 字之间',
