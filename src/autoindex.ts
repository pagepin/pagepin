...
const CODE_EXTS = new Set([
  ...
  '.py', '.go', '.rs', '.sh', '.yml', '.yaml', '.toml', '.xml', '.svg',
]);

// 文件类型显示名(meta 行 "size · TYPE")
const TYPE_LABELS: Record<string, string> = {
  '.md': 'Markdown', '.markdown': 'Markdown', '.html': 'HTML', '.htm': 'HTML',
  '.css': 'CSS', '.js': 'JavaScript', '.jsx': 'JavaScript', '.ts': 'TypeScript',
  '.tsx': 'TypeScript', '.json': 'JSON', '.png': 'PNG', '.jpg': 'JPEG', '.jpeg': 'JPEG',
  '.gif': 'GIF', '.webp': 'WebP', '.svg': 'SVG', '.avif': 'AVIF', '.pdf': 'PDF',
  '.txt': 'Text', '.yml': 'YAML', '.yaml': 'YAML', '.xml': 'XML', '.csv': 'CSV',
};
